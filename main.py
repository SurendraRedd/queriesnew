...
def get_messages():
    ...
    response = supabase.table("Mytable").select("*").order("id", desc=True).execute()
...
def render_content():
    ...
    return Div(
        ...
        Div(
            "Made with ❤️ by ",
            A("Surendra", href="https://github.com/SurendraRedd/queriesnew", target="_blank"),
        ),
        Hr(),
        render_message_list(),
    )
...
@rt("/", methods=["GET"])
def get():
    return Titled("Queries 📖", render_content())
